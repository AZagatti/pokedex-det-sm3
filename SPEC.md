...
Git remote (SSH): `https://github.com/AZagatti/pokedex-det-sm3.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-det-sm3`. The live URL will be `https://azagatti.github.io/pokedex-det-sm3/`.
